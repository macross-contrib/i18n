...
func prepareOptions(options []Options) Options {
	...
	sec := macross.Config().Section(opt.Section)
...
func I18n(options ...Options) macross.Handler {
	...
	return func(ctx *macross.Context) error {
		...
		if len(lang) == 0 {
			tags, _, _ := language.ParseAcceptLanguage(string(ctx.Request.Header.Peek("Accept-Language")))
		...
		if opt.Redirect && isNeedRedir {
			ctx.Redirect(opt.SubURL + ctx.URI().String()[:strings.Index(ctx.URI().String(), "?")])
		}
		return ctx.Next()
	}
}
